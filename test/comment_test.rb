...
class CommentApiTest < Minitest::Test
  ENTITY = "Comment"
  BASE = "/comments"
  ...
  def test_sort_by_text_accepted
    r = T.request_json(@server, "GET", "#{BASE}?sort=text&order=asc")
    assert_equal 200, r["status"]
  end
  ...
  def test_sort_by_upvote_count_numeric_with_missing_last
    ids = []
    ...
    res = T.request_json(@server, "GET", "#{BASE}?sort=upvoteCount&order=asc&limit=100")
    assert_equal 200, res["status"]
    mine = res["body"]["items"].select { |i| ids.include?(i["id"]) }
    present_vals = mine.select { |i| i["upvoteCount"].is_a?(Numeric) }.map { |i| i["upvoteCount"] }
    assert_equal [0, 9, 99, 100], present_vals
    assert_equal missing_id, mine.last["id"]
  end

  def test_filter_on_text_field_text_returns_matches
    created = create_item
    needle = (created["text"] || "").to_s[0, 4]
    return if needle.empty?
    r = T.request_json(@server, "GET", "#{BASE}?text=#{CGI.escape(needle)}")
  ...
  def test_leading_trailing_whitespace_trimmed_on_create
    ...
    assert_equal "trimmed value", r["body"]["text"]
  end
  ...
  def test_control_characters_stripped_on_create
    ...
    r = T.request_json(@server, "POST", BASE, payload)
    assert_equal 201, r["status"], "expected 201: #{r["raw"]}"
    assert_equal "cleaned", r["body"]["text"]
  end
  ...
  def test_multiline_field_text_keeps_newlines
    payload = T.build_payload(@server, ENTITY)
    payload["text"] = "first line\nsecond line"
    r = T.request_json(@server, "POST", BASE, payload)
    assert_equal 201, r["status"], "expected 201: #{r["raw"]}"
    assert_equal "first line\nsecond line", r["body"]["text"]
  end

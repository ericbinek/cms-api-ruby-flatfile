...
class WebSiteApiTest < Minitest::Test
  ENTITY = "WebSite"
  BASE = "/web-sites"
  ...
  def test_get_by_id_embeds_image_object_list_stays_flat
    ...
    assert_kind_of String, in_list["image"]
  end

  def test_get_by_id_leaves_dangling_image_ref_as_uuid
    dangling = "00000000-0000-0000-0000-000000000000"
    payload = T.build_payload(@server, ENTITY, partial: true)
    payload["image"] = dangling
    created = T.request_json(@server, "POST", BASE, payload)["body"]
    got = T.request_json(@server, "GET", "#{BASE}/#{created["id"]}")["body"]
    assert_equal dangling, got["image"]
  end

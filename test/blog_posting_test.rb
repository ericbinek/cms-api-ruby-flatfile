...
class BlogPostingApiTest < Minitest::Test
  ENTITY = "BlogPosting"
  BASE = "/blog-postings"
  ...
  def test_sort_by_headline_accepted
    r = T.request_json(@server, "GET", "#{BASE}?sort=headline&order=asc")
    assert_equal 200, r["status"]
  end
  ...
  def test_sort_by_word_count_numeric_with_missing_last
    ids = []
    [100, 0, 9, 99].each do |v|
      payload = T.build_payload(@server, ENTITY)
      payload["wordCount"] = v
      r = T.request_json(@server, "POST", BASE, payload)
      assert_equal 201, r["status"], "expected 201, got #{r["status"]}: #{r["raw"]}"
      ids << r["body"]["id"]
    end
    r0 = T.request_json(@server, "POST", BASE, T.build_payload(@server, ENTITY))
    assert_equal 201, r0["status"]
    missing_id = r0["body"]["id"]
    ids << missing_id

    res = T.request_json(@server, "GET", "#{BASE}?sort=wordCount&order=asc&limit=100")
    assert_equal 200, res["status"]
    mine = res["body"]["items"].select { |i| ids.include?(i["id"]) }
    present_vals = mine.select { |i| i["wordCount"].is_a?(Numeric) }.map { |i| i["wordCount"] }
    assert_equal [0, 9, 99, 100], present_vals
    assert_equal missing_id, mine.last["id"]
  end

  def test_filter_on_text_field_headline_returns_matches
    created = create_item
    needle = (created["headline"] || "").to_s[0, 4]
    return if needle.empty?
    r = T.request_json(@server, "GET", "#{BASE}?headline=#{CGI.escape(needle)}")
  ...
  def test_leading_trailing_whitespace_trimmed_on_create
    ...
    assert_equal "trimmed value", r["body"]["headline"]
  end
  ...
  def test_control_characters_stripped_on_create
    ...
    r = T.request_json(@server, "POST", BASE, payload)
    assert_equal 201, r["status"], "expected 201: #{r["raw"]}"
    assert_equal "cleaned", r["body"]["headline"]
  end
  ...
  def test_single_line_field_headline_strips_newlines
    payload = T.build_payload(@server, ENTITY)
    payload["headline"] = "first\nsecond"
    r = T.request_json(@server, "POST", BASE, payload)
    assert_equal 201, r["status"], "expected 201: #{r["raw"]}"
    assert_equal "firstsecond", r["body"]["headline"]
  end

  def test_get_by_id_embeds_author_object_list_stays_flat
    ...
    assert_kind_of String, in_list["author"]
  end

  def test_get_by_id_leaves_dangling_author_ref_as_uuid
  ...
  def test_get_by_id_embeds_image_as_list_of_objects
    payload = T.build_payload(@server, ENTITY, partial: true)
    created = T.request_json(@server, "POST", BASE, payload)["body"]
    got = T.request_json(@server, "GET", "#{BASE}/#{created["id"]}")["body"]
    embedded = got["image"]
    assert_kind_of Array, embedded
    assert_operator embedded.length, :>=, 1
    assert_kind_of Hash, embedded[0]
    assert_equal "ImageObject", embedded[0]["@type"]
  end

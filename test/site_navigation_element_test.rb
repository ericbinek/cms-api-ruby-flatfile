...
class SiteNavigationElementApiTest < Minitest::Test
  ...
  def test_sort_by_position_numeric_with_missing_last
    ids = []
    ...
    res = T.request_json(@server, "GET", "#{BASE}?sort=position&order=asc&limit=100")
    assert_equal 200, res["status"]
    mine = res["body"]["items"].select { |i| ids.include?(i["id"]) }
    present_vals = mine.select { |i| i["position"].is_a?(Numeric) }.map { |i| i["position"] }
  ...
  def test_get_by_id_embeds_is_part_of_object_list_stays_flat
    ...
    assert_kind_of String, in_list["isPartOf"]
  end

  def test_get_by_id_leaves_dangling_is_part_of_ref_as_uuid
    dangling = "00000000-0000-0000-0000-000000000000"
    payload = T.build_payload(@server, ENTITY, partial: true)
    payload["isPartOf"] = dangling
    created = T.request_json(@server, "POST", BASE, payload)["body"]
    got = T.request_json(@server, "GET", "#{BASE}/#{created["id"]}")["body"]
    assert_equal dangling, got["isPartOf"]
  end

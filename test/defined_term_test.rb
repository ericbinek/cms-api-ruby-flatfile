...
class DefinedTermApiTest < Minitest::Test
  ENTITY = "DefinedTerm"
  BASE = "/defined-terms"
  ...
  def test_get_by_id_embeds_in_defined_term_set_object_list_stays_flat
    ...
    assert_equal "DefinedTermSet", embedded["@type"]
    ...
    assert_kind_of String, in_list["inDefinedTermSet"]
  end

  def test_get_by_id_leaves_dangling_in_defined_term_set_ref_as_uuid
    dangling = "00000000-0000-0000-0000-000000000000"
    payload = T.build_payload(@server, ENTITY, partial: true)
    payload["inDefinedTermSet"] = dangling
    created = T.request_json(@server, "POST", BASE, payload)["body"]
    got = T.request_json(@server, "GET", "#{BASE}/#{created["id"]}")["body"]
    assert_equal dangling, got["inDefinedTermSet"]
  end

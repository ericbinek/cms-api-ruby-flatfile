...
module Cms
  module Models
    module CategoryCode
      ...
      REQUIRED_FIELDS = ["name", "codeValue", "inCodeSet"].freeze
      SEARCHABLE_FIELDS = ["name", "description", "codeValue"].freeze
      SORTABLE_FIELDS = ["dateCreated", "dateModified", "name", "description", "codeValue", "url"].freeze

      # Properties whose combined value must be unique across the collection. Empty
      # when the entity allows duplicates.
      UNIQUE_KEY = ["codeValue", "inCodeSet"].freeze

      SYSTEM_FIELDS = ["id", "dateCreated", "dateModified", "@context", "@type"].freeze

      REF_COLLECTIONS = { "CategoryCodeSet" => "category-code-sets.json" }.freeze

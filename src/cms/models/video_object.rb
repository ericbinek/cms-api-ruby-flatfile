...
module Cms
  module Models
    module VideoObject
      ...
      REQUIRED_FIELDS = ["contentUrl"].freeze
      SEARCHABLE_FIELDS = ["name", "description", "encodingFormat", "videoQuality", "transcript", "caption"].freeze
      SORTABLE_FIELDS = ["dateCreated", "dateModified", "name", "description", "contentUrl", "embedUrl", "encodingFormat", "duration", "videoQuality", "transcript", "caption", "uploadDate"].freeze

      # Properties whose combined value must be unique across the collection. Empty
      # when the entity allows duplicates.
      UNIQUE_KEY = ["contentUrl"].freeze

      SYSTEM_FIELDS = ["id", "dateCreated", "dateModified", "@context", "@type"].freeze

      REF_COLLECTIONS = { "Person" => "persons.json", "ImageObject" => "image-objects.json", "Organization" => "organizations.json" }.freeze
